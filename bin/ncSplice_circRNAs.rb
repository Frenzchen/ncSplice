...
begin	
	Analysis.prepare_anchorpairs(unmapped_reads, anchor_length, "#{prefix}_anchors.fastq")
 	...
 	CircRNA.seed_extension(anchor_pairs, anchor_length, read_length, chromosome_files, "#{prefix}_candidateReads.txt")	
 	...
 	Open3.popen3("samtools view #{prefix}_remapping.bam") do |stdin, stdout, stderr, t|
 		CircRNA.remapped_reads(stdout, "#{prefix}_remapped.txt", read_length)
 	end
 	
 	CircRNA.final_candidates("#{prefix}_candidates.txt", "#{prefix}_remapped.txt", "#{prefix}_final.txt")
